...
namespace Gurux.Serial
{
    ...
    /// See help in http://www.gurux.fi/index.php?q=Gurux.Net
    /// </summary>
    public class GXSerial : IGXMedia2, INotifyPropertyChanged, IDisposable
    {
        ...
        internal SerialPort m_base = new SerialPort();
        ...
        public GXSerial(string portName)
        {
            ConfigurableSettings = AvailableMediaSettings.All;
            m_syncBase = new GXSynchronousMediaBase(1024);
            m_base.DataReceived += new SerialDataReceivedEventHandler(GXSerial_DataReceived);
            PortName = portName;
        }
        ...
        internal void GXSerial_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            try
            {
                int count = 0;
                ...
                if (totalCount != 0 && Eop != null) //Search Eop if given.
                {
                    ...
                    if (Eop is Array)
                    {
                        foreach (object it in (Array)Eop)
                        {
                            ...
                            totalCount = Common.GXCommon.IndexOf(m_syncBase.m_Received, eop, index, m_syncBase.receivedSize);
                            if (totalCount != -1)
                            {
                                break;
                            }
        ...
        public void Open()
        {
            ...
            try
            {
                ...
                if (m_Trace >= TraceLevel.Info && m_OnTrace != null)
                {
                    ...
                    if (m_Eop is byte[] ba)
                    {
                        eop = BitConverter.ToString(ba, 0);
                    }
                    else if (m_Eop != null)
                    {
                        eop = m_Eop?.ToString();
        ...
        public async Task<bool> PropertiesAsync(INavigation navigation)
        {
        ...
        /// </summary>
        public ContentPage PropertiesForm
        {
            get
            {
                return new GXSettings(this, false);
            }
        }
        ...
        {
            byte[] value = Common.GXCommon.GetAsByteArray(data);
            lock (m_baseLock)
            {
                LastEopPos = 0;
